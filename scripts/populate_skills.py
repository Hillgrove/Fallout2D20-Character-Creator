...
def add_skills_from_csv(file_path):
        ...
        with open(file_path, newline='', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                ...
                if not existing_skill:
                    new_skill = Skill(name=row['Name'], description=row['Description'])
                    db.session.add(new_skill)
                    logging.info(f"Added new skill: {row['Name']}")
                else:
                    logging.info(f"Skill already exists: {row['Name']}")
...
if __name__ == "__main__":
    ...
    file_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'skills.csv')

    try:
        add_skills_from_csv(file_path)
